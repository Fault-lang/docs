...
The `multiple` keyword creates an unbounded set of flow instances rather than a single named one. The solver decides how many instances to use.
...
`multiple` is only valid for flows, not stocks.

### Characteristic Access: `::`

Use the `::` operator to access aggregate properties of a `multiple` set. Currently the only supported characteristic is `count` — the number of instances the solver created.

```
assume l::count < 5;
```

This restricts the solver to solutions where fewer than five instances of `faucet` were created. Without this constraint the solver may create an arbitrarily large number of instances to satisfy other invariants.
