...
---
# Time
Fault verifies system behavior over a bounded number of discrete rounds. A round is the fundamental unit of time — it represents one step of the system's evolution. The SMT solver checks that all assertions hold across every round.

The number of rounds is determined by how many steps you write in the run block — each line is one round.
...
## State Machine Time
In a `.fsystem` file, components are state machines. Each state has a boolean variable indicating whether it is currently active. States are inactive by default; the run block sets which state each component begins in.
...
A run block in a `.fsystem` file specifies the ordering relationship between components and how many rounds to check:
...
This runs 10 rounds — in odd rounds, the SMT solver considers both orderings of Arrival and Processor (mirroring `|` semantics from fspec); in even rounds, Queue runs in a fixed order. Without the run block, the state machine will execute each component in the order they are defined in the specification.
...
## Summary
The number of steps in your run block determines how deep the verification goes. Most properties are provable in 3–4 rounds, especially if you avoid declaring variables with specific values. More steps produce larger SMT formulas, which may cause performance issues or even models the solver never returns a solution for.
...
Fault ignores the run block of imported files.
